...
export const CardProject = ({ img, imgTitle, title, tag }: Props) => {
  return (
    <div className="group relative border-t pt-2 cursor-pointer">
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        {tag.split(" ").map((tag, i) => (
          <div
            key={i}
            className="rounded-full text-sm font-semibold px-2.5 py-0.5 bg-[#1e293b] text-white"
          >
            {tag}
          </div>
        ))}
      ...
      <div className="absolute top-16 transition duration-200 z-10 sm:visible delay-100 w-full scale-0 group-hover:scale-110 group-hover:-translate-y-[6vw] group-hover:translate-x-[2vw] group-hover:opacity-100 left-0 md:h-[150px] h-[220px]">
        <div className="relative w-full md:h-[150px] h-[220px]">
          <img
            ...
            decoding="async"
            data-nimg="fill"
            className="duration-700 md:h-[150px] h-[220px] ease-in-out scale-100 cursor-pointer object-cover transition duration shadow-xl rounded-t-md w-full"
            src={img}
          />
        </div>
